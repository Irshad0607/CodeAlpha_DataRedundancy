...
def get_records():
    try:
        container = get_cosmos_container()
        query = "SELECT c.id, c.classification, c.data, c.timestamp FROM c ORDER BY c.timestamp DESC"
        items = list(container.query_items(
            query=query,
            enable_cross_partition_query=True
        ))
...
def clear_records():
    try:
        container = get_cosmos_container()
        items = list(container.query_items(
            query="SELECT c.id, c.record_hash FROM c",
            enable_cross_partition_query=True
        ))
        deleted_count = 0
        for item in items:
        ...
        return jsonify({
            "status": "success",
            "message": f"Cleared {deleted_count} records from database."
        }), 200
...
def health_check():
    return jsonify({
        "status": "healthy",
        "timestamp": datetime.utcnow().isoformat(),
        "application": "Data Redundancy Removal System",
        "version": "1.0.0"
    }), 200
